//THE SCRIPT IS SOMEHOW FUCKED. THE BUTTONS ON THE RIGHT HAND SIDE DO NOT LOAD BUT LOAD FROM A MANUAL LOAD BY DOING
// EDMbassy.initUI() IN THE CONSOLE. IT APPAERS AS THOUGH THE SETTINGS SAVE AS WHEN I HAVE REOLOAED AND MANYALLY
...
EDMbassyModel = Class.extend({
	...
	populateUserlist: function() {
		...
		if ($('#side-left .sidebar-content2').children().length > 0) {
        	...
        	var timeDiv = $('<h3 title="approx. wait time until on the booth"</h3>').addClass('timewait').text('wait: ' + (spot !== null ? this.sts(decodeURIComponent(approxtime)) + ' ' : ''));
